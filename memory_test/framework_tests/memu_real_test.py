"""
MemU 真实测试模块

集成真实的MemU记忆框架进行测试
"""

import asyncio
import json
from typing import Dict, List, Any, Optional, Union, Tuple
from dataclasses import dataclass
from datetime import datetime

try:
    from memu import MemuClient
    MEMU_AVAILABLE = True
except ImportError:
    MEMU_AVAILABLE = False

from loguru import logger

from ..config import get_memory_framework_configs
from ..response_testing.memory_aware_chat import MemoryFrameworkInterface


@dataclass
class MemuMemory:
    """MemU记忆数据类"""
    memory_id: str
    content: str
    memory_type: str
    timestamp: str
    metadata: Dict[str, Any]
    relevance_score: Optional[float] = None


class MemuFrameworkAdapter(MemoryFrameworkInterface):
    """MemU框架适配器"""
    
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.client = None
        self.user_profiles: Dict[str, Dict[str, Any]] = {}  # 用户画像缓存
        self._initialize_client()
    
    def _initialize_client(self) -> None:
        """初始化MemU客户端"""
        try:
            if MEMU_AVAILABLE and not self.config.get("use_simulation", True):
                # 尝试连接真实的MemU服务
                self.client = MemuClient(
                    api_key=self.config["api_key"],
                    base_url=self.config["base_url"]
                )
                logger.info("MemU客户端初始化成功")
            else:
                logger.info("MemU使用模拟模式")
                self.client = None
        except Exception as e:
            logger.error(f"MemU客户端初始化失败: {e}")
            self.client = None
    
    async def store_conversation(self, user_id: str, user_input: str, ai_response: str) -> bool:
        """存储对话"""
        try:
            if self.client:
                # 真实存储
                return await self._store_real_conversation(user_id, user_input, ai_response)
            else:
                # 模拟存储
                return await self._store_simulated_conversation(user_id, user_input, ai_response)
        except Exception as e:
            logger.error(f"MemU存储对话失败: {e}")
            return False
    
    async def _store_real_conversation(self, user_id: str, user_input: str, ai_response: str) -> bool:
        """真实存储对话到MemU"""
        try:
            # 存储用户输入
            user_memory_result = await self.client.store_memory(
                user_id=user_id,
                memory_type="user_input",
                content=user_input,
                metadata={
                    "timestamp": datetime.now().isoformat(),
                    "session_type": "divination_consultation"
                }
            )
            
            # 存储AI回复
            ai_memory_result = await self.client.store_memory(
                user_id=user_id,
                memory_type="ai_response", 
                content=ai_response,
                metadata={
                    "timestamp": datetime.now().isoformat(),
                    "related_input": user_input[:100]  # 关联的用户输入摘要
                }
            )
            
            success = user_memory_result and ai_memory_result
            logger.debug(f"MemU真实存储成功: {user_id}")
            return success
            
        except Exception as e:
            logger.error(f"MemU真实存储失败: {e}")
            return False
    
    async def _store_simulated_conversation(self, user_id: str, user_input: str, ai_response: str) -> bool:
        """模拟存储对话"""
        # 模拟存储逻辑
        memory_key = f"memu_sim_{user_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        # 创建模拟记忆条目
        simulated_memory = {
            "id": memory_key,
            "user_id": user_id,
            "content": f"对话记录\\n用户: {user_input}\\n助手: {ai_response}",
            "type": "conversation",
            "timestamp": datetime.now().isoformat(),
            "metadata": {
                "framework": "memu",
                "simulation": True,
                "user_input_length": len(user_input),
                "ai_response_length": len(ai_response)
            }
        }
        
        # 存储到内存缓存中
        if not hasattr(self, 'simulated_memories'):
            self.simulated_memories = {}
        
        if user_id not in self.simulated_memories:
            self.simulated_memories[user_id] = []
        
        self.simulated_memories[user_id].append(simulated_memory)
        
        # 同时创建分析记忆（MemU特色功能模拟）
        analysis_memory = {
            "id": f"{memory_key}_analysis",
            "user_id": user_id,
            "content": f"对话分析: 用户关注{self._extract_keywords(user_input)}, AI提供了{self._analyze_response_type(ai_response)}",
            "type": "analysis",
            "timestamp": datetime.now().isoformat(),
            "metadata": {
                "framework": "memu",
                "simulation": True,
                "analysis_type": "conversation_pattern"
            }
        }
        
        self.simulated_memories[user_id].append(analysis_memory)
        
        logger.debug(f"MemU模拟存储成功: {user_id}")
        return True
    
    def _extract_keywords(self, text: str) -> str:
        """提取关键词（简化实现）"""
        keywords = ["工作", "感情", "财运", "健康", "事业", "婚姻", "投资", "学习"]
        found_keywords = [keyword for keyword in keywords if keyword in text]
        return "、".join(found_keywords) if found_keywords else "一般咨询"
    
    def _analyze_response_type(self, text: str) -> str:
        """分析回复类型（简化实现）"""
        if "建议" in text:
            return "实用建议"
        elif "分析" in text:
            return "详细分析"
        elif "预测" in text:
            return "未来预测"
        else:
            return "一般指导"
    
    async def retrieve_relevant_memories(self, user_id: str, query: str) -> List[Dict[str, Any]]:
        """检索相关记忆"""
        try:
            if self.client:
                return await self._retrieve_real_memories(user_id, query)
            else:
                return await self._retrieve_simulated_memories(user_id, query)
        except Exception as e:
            logger.error(f"MemU检索记忆失败: {e}")
            return []
    
    async def _retrieve_real_memories(self, user_id: str, query: str) -> List[Dict[str, Any]]:
        """从真实MemU检索记忆"""
        try:
            # 使用MemU的检索功能
            results = await self.client.search_memories(
                user_id=user_id,
                query=query,
                memory_types=["user_input", "ai_response", "analysis"],
                limit=10
            )
            
            memories = []
            for result in results:
                memory = {
                    "id": result.get("memory_id", ""),
                    "content": result.get("content", ""),
                    "type": result.get("memory_type", "unknown"),
                    "timestamp": result.get("created_at", ""),
                    "relevance_score": result.get("relevance_score", 0.0),
                    "metadata": result.get("metadata", {})
                }
                memories.append(memory)
            
            logger.debug(f"MemU真实检索: {user_id}, 结果数: {len(memories)}")
            return memories
            
        except Exception as e:
            logger.error(f"MemU真实检索失败: {e}")
            return []
    
    async def _retrieve_simulated_memories(self, user_id: str, query: str) -> List[Dict[str, Any]]:
        """模拟检索记忆"""
        if not hasattr(self, 'simulated_memories'):
            self.simulated_memories = {}
        
        user_memories = self.simulated_memories.get(user_id, [])
        
        if not user_memories:
            return []
        
        # MemU风格的语义搜索模拟
        query_words = set(query.lower().split())
        relevant_memories = []
        
        for memory in user_memories:
            content = memory["content"].lower()
            content_words = set(content.split())
            
            # 计算相关性分数（MemU可能有更复杂的算法）
            intersection = query_words.intersection(content_words)
            relevance_score = 0.0
            
            if intersection:
                # 基础匹配分数
                base_score = len(intersection) / len(query_words.union(content_words))
                
                # MemU特色：根据记忆类型调整分数
                type_bonus = 0.0
                memory_type = memory.get("type", "")
                if memory_type == "analysis":
                    type_bonus = 0.2  # 分析类记忆获得加分
                elif memory_type == "conversation":
                    type_bonus = 0.1
                
                # 时间衰减
                try:
                    memory_time = datetime.fromisoformat(memory["timestamp"])
                    time_diff = (datetime.now() - memory_time).days
                    time_decay = max(0.1, 1.0 - (time_diff / 180))  # 半年内的记忆
                except:
                    time_decay = 0.5
                
                relevance_score = (base_score + type_bonus) * time_decay
            
            if relevance_score > 0.15:  # 设置阈值
                memory_copy = memory.copy()
                memory_copy["relevance_score"] = relevance_score
                relevant_memories.append(memory_copy)
        
        # 按相关性排序
        relevant_memories.sort(key=lambda x: x["relevance_score"], reverse=True)
        
        logger.debug(f"MemU模拟检索: {user_id}, 结果数: {len(relevant_memories)}")
        return relevant_memories[:8]  # 返回最相关的8个记忆
    
    async def get_user_profile(self, user_id: str) -> Dict[str, Any]:
        """获取用户画像"""
        try:
            if self.client:
                return await self._get_real_user_profile(user_id)
            else:
                return await self._get_simulated_user_profile(user_id)
        except Exception as e:
            logger.error(f"MemU获取用户画像失败: {e}")
            return {}
    
    async def _get_real_user_profile(self, user_id: str) -> Dict[str, Any]:
        """从真实MemU获取用户画像"""
        try:
            profile = await self.client.get_user_profile(user_id)
            return profile if profile else {}
        except Exception as e:
            logger.error(f"MemU真实用户画像获取失败: {e}")
            return {}
    
    async def _get_simulated_user_profile(self, user_id: str) -> Dict[str, Any]:
        """模拟获取用户画像"""
        # 返回缓存的用户画像或创建默认画像
        if user_id in self.user_profiles:
            return self.user_profiles[user_id]
        
        # 创建MemU风格的默认用户画像
        default_profile = {
            "user_id": user_id,
            "personality_traits": ["理性", "求知欲强"],
            "communication_style": "直接",
            "concerns": ["个人发展", "决策支持"],
            "interaction_patterns": {
                "session_frequency": "irregular",
                "question_types": ["general_inquiry"],
                "preferred_response_length": "medium"
            },
            "memory_stats": {
                "total_memories": 0,
                "memory_types": {},
                "last_interaction": None
            },
            "framework": "memu_simulation",
            "created_at": datetime.now().isoformat()
        }
        
        self.user_profiles[user_id] = default_profile
        return default_profile
    
    async def update_user_profile(self, user_id: str, updates: Dict[str, Any]) -> bool:
        """更新用户画像"""
        try:
            if self.client:
                return await self._update_real_user_profile(user_id, updates)
            else:
                return await self._update_simulated_user_profile(user_id, updates)
        except Exception as e:
            logger.error(f"MemU更新用户画像失败: {e}")
            return False
    
    async def _update_real_user_profile(self, user_id: str, updates: Dict[str, Any]) -> bool:
        """更新真实MemU用户画像"""
        try:
            result = await self.client.update_user_profile(user_id, updates)
            return result is not None
        except Exception as e:
            logger.error(f"MemU真实用户画像更新失败: {e}")
            return False
    
    async def _update_simulated_user_profile(self, user_id: str, updates: Dict[str, Any]) -> bool:
        """模拟更新用户画像"""
        # 确保用户画像存在
        if user_id not in self.user_profiles:
            await self._get_simulated_user_profile(user_id)
        
        # 更新画像
        profile = self.user_profiles[user_id]
        
        for key, value in updates.items():
            if key == "interaction_patterns":
                # 特殊处理交互模式
                if "interaction_patterns" not in profile:
                    profile["interaction_patterns"] = {}
                profile["interaction_patterns"].update(value)
            elif key == "memory_stats":
                # 特殊处理记忆统计
                if "memory_stats" not in profile:
                    profile["memory_stats"] = {}
                profile["memory_stats"].update(value)
            elif key in profile and isinstance(profile[key], list) and isinstance(value, list):
                # 列表合并
                combined = list(set(profile[key] + value))
                profile[key] = combined[:5]  # 限制长度
            else:
                profile[key] = value
        
        profile["updated_at"] = datetime.now().isoformat()
        
        # 更新记忆统计
        if hasattr(self, 'simulated_memories') and user_id in self.simulated_memories:
            memories = self.simulated_memories[user_id]
            profile["memory_stats"]["total_memories"] = len(memories)
            
            # 统计记忆类型
            type_counts = {}
            for memory in memories:
                mem_type = memory.get("type", "unknown")
                type_counts[mem_type] = type_counts.get(mem_type, 0) + 1
            profile["memory_stats"]["memory_types"] = type_counts
            
            if memories:
                profile["memory_stats"]["last_interaction"] = memories[-1]["timestamp"]
        
        logger.debug(f"MemU模拟用户画像更新成功: {user_id}")
        return True


class MemuRealTester:
    """MemU真实测试器"""
    
    def __init__(self):
        self.config = get_memory_framework_configs()["memu"]
        self.framework_adapter = MemuFrameworkAdapter(self.config)
        self.test_sessions: Dict[str, Dict[str, Any]] = {}
    
    def create_test_session(self, 
                          test_name: str, 
                          user_profile: Dict[str, Any]) -> str:
        """创建测试会话"""
        
        session_id = f"memu_test_{test_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        session = {
            "session_id": session_id,
            "test_name": test_name,
            "user_profile": user_profile,
            "start_time": datetime.now().isoformat(),
            "interactions": [],
            "framework_type": "memu"
        }
        
        self.test_sessions[session_id] = session
        
        logger.info(f"MemU测试会话已创建: {session_id}")
        return session_id
    
    async def test_memory_storage_and_retrieval(self, 
                                              session_id: str,
                                              test_conversations: List[Tuple[str, str]]) -> Dict[str, Any]:
        """测试记忆存储和检索功能"""
        
        if session_id not in self.test_sessions:
            raise ValueError(f"测试会话 {session_id} 不存在")
        
        session = self.test_sessions[session_id]
        user_id = f"test_user_{session_id}"
        
        results = {
            "session_id": session_id,
            "user_id": user_id,
            "storage_results": [],
            "retrieval_results": [],
            "performance_metrics": {}
        }
        
        # 测试存储功能
        logger.info(f"开始测试MemU存储功能: {session_id}")
        
        storage_start_time = datetime.now()
        storage_success_count = 0
        
        for i, (user_input, ai_response) in enumerate(test_conversations):
            storage_success = await self.framework_adapter.store_conversation(
                user_id, user_input, ai_response
            )
            
            storage_result = {
                "conversation_index": i,
                "user_input": user_input,
                "ai_response": ai_response,
                "storage_success": storage_success,
                "timestamp": datetime.now().isoformat()
            }
            
            results["storage_results"].append(storage_result)
            
            if storage_success:
                storage_success_count += 1
            
            # 记录到会话
            session["interactions"].append({
                "type": "storage_test",
                "data": storage_result
            })
        
        storage_duration = (datetime.now() - storage_start_time).total_seconds()
        
        # 测试检索功能 - MemU特色查询
        logger.info(f"开始测试MemU检索功能: {session_id}")
        
        retrieval_start_time = datetime.now()
        
        # 使用不同类型的查询测试MemU检索
        test_queries = [
            "事业发展建议",
            "感情方面的咨询",
            "财务投资决策",
            "健康养生指导",
            "学习成长规划",
            "人际关系处理",
            "最近的对话内容"
        ]
        
        for query in test_queries:
            retrieval_start = datetime.now()
            memories = await self.framework_adapter.retrieve_relevant_memories(user_id, query)
            retrieval_time = (datetime.now() - retrieval_start).total_seconds()
            
            # 分析检索到的记忆质量
            memory_quality = self._analyze_memory_quality(memories, query)
            
            retrieval_result = {
                "query": query,
                "memories_found": len(memories),
                "memories": memories,
                "retrieval_time": retrieval_time,
                "memory_quality": memory_quality,
                "timestamp": datetime.now().isoformat()
            }
            
            results["retrieval_results"].append(retrieval_result)
            
            # 记录到会话
            session["interactions"].append({
                "type": "retrieval_test",
                "data": retrieval_result
            })
        
        retrieval_duration = (datetime.now() - retrieval_start_time).total_seconds()
        
        # 计算性能指标
        results["performance_metrics"] = {
            "storage_success_rate": storage_success_count / len(test_conversations),
            "total_storage_time": storage_duration,
            "avg_storage_time": storage_duration / len(test_conversations),
            "total_retrieval_time": retrieval_duration,
            "avg_retrieval_time": retrieval_duration / len(test_queries),
            "avg_memories_per_query": sum(len(r["memories"]) for r in results["retrieval_results"]) / len(test_queries),
            "avg_memory_quality": sum(r["memory_quality"]["overall_score"] for r in results["retrieval_results"]) / len(test_queries)
        }
        
        logger.info(f"MemU存储检索测试完成: {session_id}")
        return results
    
    def _analyze_memory_quality(self, memories: List[Dict[str, Any]], query: str) -> Dict[str, Any]:
        """分析记忆质量（MemU特色功能）"""
        if not memories:
            return {
                "overall_score": 0.0,
                "relevance_score": 0.0,
                "diversity_score": 0.0,
                "recency_score": 0.0,
                "type_distribution": {}
            }
        
        # 相关性分析
        relevance_scores = [memory.get("relevance_score", 0.0) for memory in memories]
        avg_relevance = sum(relevance_scores) / len(relevance_scores)
        
        # 多样性分析
        memory_types = [memory.get("type", "unknown") for memory in memories]
        unique_types = len(set(memory_types))
        diversity_score = min(1.0, unique_types / 3)  # 3种类型以上为满分
        
        # 时效性分析
        now = datetime.now()
        recency_scores = []
        for memory in memories:
            try:
                memory_time = datetime.fromisoformat(memory.get("timestamp", ""))
                days_diff = (now - memory_time).days
                recency_score = max(0.0, 1.0 - days_diff / 30)  # 30天内为新记忆
                recency_scores.append(recency_score)
            except:
                recency_scores.append(0.0)
        
        avg_recency = sum(recency_scores) / len(recency_scores) if recency_scores else 0.0
        
        # 类型分布统计
        type_distribution = {}
        for mem_type in memory_types:
            type_distribution[mem_type] = type_distribution.get(mem_type, 0) + 1
        
        # 综合评分
        overall_score = (avg_relevance * 0.5 + diversity_score * 0.25 + avg_recency * 0.25)
        
        return {
            "overall_score": overall_score,
            "relevance_score": avg_relevance,
            "diversity_score": diversity_score,
            "recency_score": avg_recency,
            "type_distribution": type_distribution
        }
    
    async def test_user_profile_management(self, 
                                         session_id: str,
                                         profile_updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        """测试用户画像管理功能"""
        
        if session_id not in self.test_sessions:
            raise ValueError(f"测试会话 {session_id} 不存在")
        
        user_id = f"test_user_{session_id}"
        
        results = {
            "session_id": session_id,
            "user_id": user_id,
            "initial_profile": None,
            "update_results": [],
            "final_profile": None,
            "profile_evolution": []
        }
        
        # 获取初始用户画像
        initial_profile = await self.framework_adapter.get_user_profile(user_id)
        results["initial_profile"] = initial_profile
        
        logger.info(f"开始测试MemU用户画像管理: {session_id}")
        
        # 测试画像更新
        for i, update_data in enumerate(profile_updates):
            update_start = datetime.now()
            update_success = await self.framework_adapter.update_user_profile(user_id, update_data)
            update_time = (datetime.now() - update_start).total_seconds()
            
            # 验证更新结果
            updated_profile = await self.framework_adapter.get_user_profile(user_id)
            
            # 分析画像变化
            profile_change_analysis = self._analyze_profile_changes(
                initial_profile, updated_profile
            )
            
            update_result = {
                "update_index": i,
                "update_data": update_data,
                "update_success": update_success,
                "update_time": update_time,
                "profile_after_update": updated_profile,
                "change_analysis": profile_change_analysis,
                "timestamp": datetime.now().isoformat()
            }
            
            results["update_results"].append(update_result)
            results["profile_evolution"].append({
                "step": i,
                "profile_snapshot": updated_profile.copy(),
                "timestamp": datetime.now().isoformat()
            })
        
        # 获取最终用户画像
        final_profile = await self.framework_adapter.get_user_profile(user_id)
        results["final_profile"] = final_profile
        
        logger.info(f"MemU用户画像管理测试完成: {session_id}")
        return results
    
    def _analyze_profile_changes(self, 
                               initial_profile: Dict[str, Any], 
                               updated_profile: Dict[str, Any]) -> Dict[str, Any]:
        """分析画像变化"""
        changes = {
            "added_fields": [],
            "modified_fields": [],
            "removed_fields": [],
            "change_count": 0
        }
        
        # 检查新增字段
        for key in updated_profile:
            if key not in initial_profile:
                changes["added_fields"].append(key)
                changes["change_count"] += 1
        
        # 检查修改和删除的字段
        for key in initial_profile:
            if key not in updated_profile:
                changes["removed_fields"].append(key)
                changes["change_count"] += 1
            elif initial_profile[key] != updated_profile[key]:
                changes["modified_fields"].append({
                    "field": key,
                    "old_value": initial_profile[key],
                    "new_value": updated_profile[key]
                })
                changes["change_count"] += 1
        
        return changes
    
    async def test_integration_with_ai_chat(self, 
                                          session_id: str,
                                          chat_sequence: List[str]) -> Dict[str, Any]:
        """测试与AI聊天的集成"""
        
        from ..response_testing.memory_aware_chat import MemoryAwareChat
        
        if session_id not in self.test_sessions:
            raise ValueError(f"测试会话 {session_id} 不存在")
        
        session = self.test_sessions[session_id]
        user_id = f"test_user_{session_id}"
        
        # 创建记忆感知聊天系统
        memory_chat = MemoryAwareChat("memu", self.framework_adapter)
        
        results = {
            "session_id": session_id,
            "user_id": user_id,
            "chat_interactions": [],
            "integration_metrics": {},
            "memory_evolution": []
        }
        
        logger.info(f"开始测试MemU与AI聊天集成: {session_id}")
        
        # 开始对话
        chat_session_id = await memory_chat.start_conversation(user_id, session["user_profile"])
        
        integration_start_time = datetime.now()
        
        # 执行聊天序列
        for i, user_message in enumerate(chat_sequence):
            interaction_start = datetime.now()
            
            # 记录交互前的记忆状态
            pre_memories = await self.framework_adapter.retrieve_relevant_memories(user_id, user_message)
            
            try:
                ai_response = await memory_chat.send_message(user_id, user_message)
                interaction_time = (datetime.now() - interaction_start).total_seconds()
                
                # 记录交互后的记忆状态
                post_memories = await self.framework_adapter.retrieve_relevant_memories(user_id, user_message)
                
                # 分析记忆演化
                memory_evolution = self._analyze_memory_evolution(pre_memories, post_memories)
                
                interaction_result = {
                    "interaction_index": i,
                    "user_message": user_message,
                    "ai_response": ai_response.ai_response,
                    "response_time": ai_response.response_time,
                    "interaction_time": interaction_time,
                    "memory_context_used": bool(ai_response.memory_context),
                    "memory_context_size": len(ai_response.memory_context),
                    "memory_evolution": memory_evolution,
                    "success": True,
                    "timestamp": ai_response.timestamp
                }
                
            except Exception as e:
                interaction_time = (datetime.now() - interaction_start).total_seconds()
                interaction_result = {
                    "interaction_index": i,
                    "user_message": user_message,
                    "error": str(e),
                    "interaction_time": interaction_time,
                    "success": False,
                    "timestamp": datetime.now().isoformat()
                }
                logger.error(f"MemU聊天集成测试交互失败: {e}")
            
            results["chat_interactions"].append(interaction_result)
            
            # 记录记忆演化
            if interaction_result["success"]:
                results["memory_evolution"].append({
                    "interaction_index": i,
                    "memory_count_before": len(pre_memories),
                    "memory_count_after": len(post_memories) if 'post_memories' in locals() else 0,
                    "evolution_analysis": memory_evolution if 'memory_evolution' in locals() else {}
                })
        
        total_integration_time = (datetime.now() - integration_start_time).total_seconds()
        
        # 结束对话并获取统计信息
        conversation_stats = await memory_chat.end_conversation(user_id)
        
        # 计算集成指标
        successful_interactions = [r for r in results["chat_interactions"] if r["success"]]
        
        results["integration_metrics"] = {
            "total_interactions": len(chat_sequence),
            "successful_interactions": len(successful_interactions),
            "success_rate": len(successful_interactions) / len(chat_sequence),
            "total_integration_time": total_integration_time,
            "avg_interaction_time": total_integration_time / len(chat_sequence),
            "memory_utilization_rate": sum(1 for r in successful_interactions if r.get("memory_context_used", False)) / len(successful_interactions) if successful_interactions else 0,
            "memory_growth_rate": self._calculate_memory_growth_rate(results["memory_evolution"]),
            "conversation_stats": conversation_stats
        }
        
        logger.info(f"MemU与AI聊天集成测试完成: {session_id}")
        return results
    
    def _analyze_memory_evolution(self, 
                                pre_memories: List[Dict[str, Any]], 
                                post_memories: List[Dict[str, Any]]) -> Dict[str, Any]:
        """分析记忆演化"""
        pre_ids = set(memory.get("id", "") for memory in pre_memories)
        post_ids = set(memory.get("id", "") for memory in post_memories)
        
        new_memories = post_ids - pre_ids
        retained_memories = pre_ids.intersection(post_ids)
        
        return {
            "memory_count_before": len(pre_memories),
            "memory_count_after": len(post_memories),
            "new_memories_count": len(new_memories),
            "retained_memories_count": len(retained_memories),
            "memory_turnover_rate": len(new_memories) / max(1, len(post_memories)) if post_memories else 0
        }
    
    def _calculate_memory_growth_rate(self, memory_evolution: List[Dict[str, Any]]) -> float:
        """计算记忆增长率"""
        if not memory_evolution:
            return 0.0
        
        total_growth = 0
        for evolution in memory_evolution:
            before = evolution.get("memory_count_before", 0)
            after = evolution.get("memory_count_after", 0)
            if before > 0:
                growth_rate = (after - before) / before
                total_growth += growth_rate
        
        return total_growth / len(memory_evolution)
    
    def get_session_summary(self, session_id: str) -> Optional[Dict[str, Any]]:
        """获取测试会话摘要"""
        if session_id not in self.test_sessions:
            return None
        
        session = self.test_sessions[session_id]
        
        return {
            "session_info": {
                "session_id": session_id,
                "test_name": session["test_name"],
                "framework_type": session["framework_type"],
                "start_time": session["start_time"],
                "user_profile": session["user_profile"]
            },
            "interaction_count": len(session["interactions"]),
            "interaction_types": list(set(interaction["type"] for interaction in session["interactions"])),
            "framework_config": self.config
        }
    
    def export_test_results(self, session_id: str, file_path: str) -> bool:
        """导出测试结果"""
        summary = self.get_session_summary(session_id)
        if not summary:
            return False
        
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(summary, f, ensure_ascii=False, indent=2)
            logger.info(f"MemU测试结果已导出: {file_path}")
            return True
        except Exception as e:
            logger.error(f"导出MemU测试结果失败: {e}")
            return False